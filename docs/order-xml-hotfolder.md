...
| `total_amount`          | computed sum of (`Quantity` × `UnitPrice`) across all line items, **plus stated `Order/ShippingTotal` when it is a finite number** (2026-08-19), rounded to 2 dp. Omitted entirely when no line item carries `UnitPrice` — a stated shipping value alone will NOT produce a total. Blank / non-numeric ShippingTotal contributes nothing (OrderHub then supplies both the shipping cost and the total's shipping portion from the matched Shipping Method at its end). Deliberately DIVERGES from PhotoFinale, whose `total_amount` is a wholesale rollup — see comment in `roes.js` for the rationale (short version: ROES has no wholesale/retail split, PhotoFinale does). |
| `notes`                 | `Order/SpecialInstructions` (omitted if empty)               |

...
- **Size**: ~576 MB (Electron + Chromium + bundled AI models). Recorded
  ...
  - `1.16.1` (2026-09-06) — 576,729,947 bytes (+8,126 vs 1.16.0 — Fuji JobMaker network image path: new `fujiImageRoot` controller field lets the path OHD writes images to differ from the path OHD writes into the emitted `.txt`'s `ImagePath=` line, needed when OHD and Fuji JobMaker run on different machines. Migration pre-fills the new field from imageStagingRoot on read so existing controllers save with no operator action (tripwire test locks byte-identical .txt for the same-machine case). Save-time reachability check is advisory (warn, never block — the 1.15.1 lesson). Dispatch-time check is strict on the discriminable case (root resolves from OHD but order subfolder missing = hard fail with actionable message) and lenient when OHD cannot see the share at all (warn + proceed — Fuji becomes the authoritative check). Also carries the Perfectly Clear batch setup DeadlineError fix from Unreleased.) **[BUILT NOT DISTRIBUTED — 1.16.1's changes reach labs via 1.16.2.]**
  - `1.16.2` (2026-09-07) — 576,737,583 bytes (+7,636 vs 1.16.1 — Folder Copy release: five items — never-overwrite guarantee at dispatch (`dedupeAgainstDisk` walks _2/_3 against on-disk state so OHD never replaces a file in a Folder Copy destination; pre-1.16.2 idempotence-on-retry replaced by never-overwrite invariant, deliberate swap); save-time hard blocks on Root+blank / Root+no-distinguisher softened to advisories (the 1.15.0 lesson — dispatch dedupe covers it); new `omitJobId` checkbox under Per-job layout, default ON for new controllers OFF for existing (strict-`=== true` migration, both route literals with parity test); default filename template `{lastName}_{jobName}_{category}_{productCode}_{quantity}_{indexPadded}` on new folder_copy controllers (existing controllers open with their saved value); token chips insert-at-cursor (was clipboard-copy) with new tooltip wording; filename template field roughly 2× height with single-line semantics preserved via input-time newline-strip. Ships 1.16.1's Fuji JobMaker network image path + Perfectly Clear DeadlineError fix — first release with those changes to reach labs.)
  - `1.16.4` (2026-09-09) — 576,754,623 bytes (+2,201 vs 1.16.3 — persistence hotfix for the 1.16.3 FTP settings defect: `ftpKeepFilesOnServer`, `ftpRetentionSweepEnabled`, `ftpRetentionSweepDays`, `ftpRetentionSweepDryRun` were added to the renderer + runtime + tests in 1.16.3 but never wired through `config-service.js`'s explicit schema / `save()` / `getAll()` per-key list, so Settings reported "saved" but silently dropped every value. Result: 1.16.3 installs looked like 1.16.2 at runtime (Copy mode off, retention sweep never ran). This release adds all four to the schema (booleans through `Boolean(...)` so `false` survives, days clamped to integer ≥ 1 with default 7 restored on garbage) plus the internal `ftpLastSweepAt` marker to the schema for discoverability. Closes the CLASS of defect with two new tests: round-trip through `save()` + `getAll()` for each key (including that `false` survives as `false` and garbage `ftpRetentionSweepDays` restores the default), and a source-scan tripwire that derives the persisted-fields list from index.html and asserts each `name="…"` input inside `#settingsForm` appears in BOTH `save()` and `getAll()` — six known UI-to-storage-mapping fields (`aiQualityHoldAutoPrint` → `aiQualityMode`, five `pc*` → `perfectlyClear` aggregate) carried on a documented NON_PERSISTED exception list. Audit confirmed the four FTP keys were the only config.json additions since 1.16.0; per-controller routing.json settings (`omitJobId`, `fujiImageRoot`) use whole-object spread in `saveController()` so the enumerated-save-list gap doesn't exist there.)
  - `1.16.3` (2026-09-09) — 576,752,422 bytes (+14,839 vs 1.16.2 — three-feature release: (1) FTP Copy mode for multi-location labs sharing one FTP folder — new `ftpKeepFilesOnServer` setting suppresses per-file DELE + parent RMD in `_downloadDirectory`, off by default so single-location labs unaffected; plus an optional retention sweep (`ftpRetentionSweepEnabled/Days/DryRun`) with 7-day default, root-path refusal, local-verify-first, 24h-throttle persisted via `ftpLastSweepAt`, and a save-time advisory when enabled with Remote Path "/". Known limitation documented (location offline > window permanently misses those orders — visible/benign failure, not silent). (2) Global "Default Folder" (`processFolderPath`) removed — Settings → Downloads → Process Folders section deleted, `defaultFolderFallback` in resolveRoute gone, unrouted jobs surface in the Jobs grid as "No routing — assign a controller for this process in Settings → Routing" in place of the Process button. Motivation: the fallback was actively hiding the deleted-controller defect (mapping pointing at a nonexistent controllerId silently copied jobs to the default folder + marked Completed, no printer ever printed). Also fixes the two-store divergence (`config-service` was writing to `config.json` while `resolveRoute` read from `routing.json` — post-migration operator edits never reached the runtime resolver). Also: stale routing-fallback-note under Settings → Routing rewritten to match new behaviour; source-scan tripwire added. (3) Configuration modals no longer close mid-edit and discard operator work — backdrop dismiss now requires mousedown + mouseup + click targets all on the overlay (was: click target only, so scrollbar-drag and text-select-overshoot dismissed), Escape and genuine backdrop click prompt "Discard unsaved changes in this form?" via a receiver-pinned dirty snapshot taken at `openModal()`, Cancel and × unchanged, Escape closes only the topmost modal. New `src/renderer/modal-dismiss.js` UMD helper + 27 unit tests (renderer previously outside the test globs — the class of defect that survived shipping cannot repeat silently). Five source-scan tripwires (script tag ordering, confirm string locked, openModal enforcement per pm-modal-overlay id) hardened via a repair pass that pinned the receiver so a `.classList.remove('hidden')` on any inner element is ignored. Also carries the 1.15.3 field-confirmation for the two cross-volume PIC Pro safety hypotheses (single successful dispatch at the trigger lab on 2026-09-09 — narrow happy-path claim; mid-copy failure branch, startup sweep, 1.16.1 reachability check NOT verified). Full write-up in the v1.16.3 CHANGELOG entry.)
  - `1.16.0` (2026-09-02) — 576,721,821 bytes (+51,983 vs 1.15.3 — PDF & image imposition release for PDF Copy controllers: M1–M10 arc adds paper-size + template stores, layout + compose engines (pure geometry, pdf-lib composition), Settings UI with live preview, dispatch wiring under a per-controller `applyImpositions: false` default so no existing controller changes on upgrade, master-sheet mode for proof-then-multiply workflows, JPEG/PNG artwork via `image-artwork.js` with stretch-to-cell for guaranteed cut alignment plus CMYK reject / 150-DPI warn / EXIF-rotation-is-a-known-limitation, output destination + filename template with `{qty}` and `{impqty}` tokens, and an operator-guide walkthrough. Carries forward the 1.15.3 Fuji PIC Pro cross-volume + silent-stall fixes via the 9b7390e merge of hotfix/1.15.3 into main. Imposition has NOT yet been press-tested at any lab — the operator notes carry a "Help us prove this" ask for the first lab to fold a duplex sheet and report front-to-back registration.)
